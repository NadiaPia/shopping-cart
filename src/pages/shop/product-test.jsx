...
useEffect(() => {
  ...
  const addToCart = (id) => {
    ...
    quantity < 1 ? (
      axios.post(`${process.env.REACT_APP_BACKEND_URL}/carts/${id}`, { userId: props.authState.id, quantity: 1 }).then((response) => {
      ...
      axios.put(`${process.env.REACT_APP_BACKEND_URL}/carts/${id}`, { userId: props.authState.id, quantity: quantity + 1 }).then((response) => {
